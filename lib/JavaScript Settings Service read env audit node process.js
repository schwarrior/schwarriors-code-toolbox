...
export class SettingService {
	...
	getProgramAudit = (withConsoleLog) => {
	...
	getAppSpecificAuditLines = () => {
		...
		if (this.settings.length < 1) {
			auditLines.push('No .env file found in working dir');
			return auditLines;
		}
		auditLines.push(this.getEnvSettingAuditLine('ENVIRONMENT'));
		auditLines.push(this.getEnvSettingAuditLine('LOG'));
		auditLines.push(this.getEnvSettingAuditLine('AzureTableStoreAcctName'));
		auditLines.push(this.getEnvSettingAuditLine('AzureTableStoreAcctKey'));
		return auditLines;
	}
	...
	getEnvSettingAuditLine = (settingName) => {
		...
		if (!value) {
			return `.env ${settingName} not found`;
		}
